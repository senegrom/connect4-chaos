...
class OutcomeTable {
 public:
  ...
  bool probe(std::uint64_t key, int& lower, int& upper) {
    ...
    ++hits;
    lower = (flags_[slot] & 1) == 0 ? -2 : lower_[slot];
    upper = (flags_[slot] & 2) == 0 ? 2 : upper_[slot];
    return true;
  }
...
class ExactSolver {
 public:
  ...
  MoveChoice choose(const Position& position) {
    ...
    if (safe == 0) {
      return {LOSS, closureCandidates(position, possible).values[0].column};
    }
    ...
    for (const Candidate& candidate : candidates) {
      const int child = solve(play(position, candidate.move));
      const int moveValue = child == DRAW ? DRAW : -child;
      if (moveValue == value) return {value, candidate.column};
    }
 ...
 private:
  ...
  CandidateList orderedMoves(const Position& position, std::uint64_t moves) {
    ...
    for (int orderIndex = 0; orderIndex < geometry_.columns; ++orderIndex) {
      ...
      const int futureWins = __builtin_popcountll(
      ...
      Candidate candidate{
          ...
          move,
          historySlot,
          history_[historySlot] + futureWins * 128 + centrality,
      };
  ...
  int continuationCount(const Position& position, const Candidate& candidate) {
    ...
    for (int index = 0; index < geometry_.columns; ++index) {
      ...
      if (hasAlignment(geometry_, opponent.current | reply)) continue;
  ...
  CandidateList closureCandidates(const Position& position, std::uint64_t moves) {
    ...
    for (int index = 0; index < candidates.count; ++index) {
      continuationCounts[index] = continuationCount(position, candidates.values[index]);
    }
    for (int index = 1; index < candidates.count; ++index) {
      ...
      while (insertion > 0) {
        ...
        const Candidate& previous = candidates.values[insertion - 1];
        if (previousContinuations < continuations
            || (previousContinuations == continuations
                && (previous.order > candidate.order
                    || (previous.order == candidate.order
                        && previous.column < candidate.column)))) break;
        candidates.values[insertion] = previous;
  ...
  int search(const Position& rawPosition, int alpha, int beta) {
    ...
    for (const Candidate& candidate : candidates) {
      const int child = search(play(position, candidate.move), -beta, -alpha);
      const int score = child == DRAW ? DRAW : -child;
      if (score >= beta) {
        ++cutoffs;
...
GeneratedPolicy generatePolicy(
  ...
  const int firstPlayerValue = solver.solve(Position{});
  const int rootValue = role == ROLE_FIRST ? firstPlayerValue : -firstPlayerValue;
