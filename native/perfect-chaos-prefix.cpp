...
int main(int argc,char**argv){try{if(argc<2)throw std::runtime_error("Usage: perfect-chaos-prefix <verify|generate|extend> ...");std::string c=argv[1];if(c=="verify")prefix::verify(argc,argv);else if(c=="generate")prefix::command_generate(argc,argv);else if(c=="extend")prefix::command_extend(argc,argv);else throw std::runtime_error("Unknown command: "+c);return 0;}catch(const std::exception&e){std::cerr<<e.what()<<'\n';return 1;}}
