...
#        [-Window] [-MinNew] [-Sims] [-ArenaEvery] [-ArenaLag] [-Shapes] [-TargetSims] [-TargetShare] [-Entropy]
...
param([string]$Init, [int]$Gen, [int]$K = 3, [int]$Games = 4096, [int]$Steps = 6000,
      ...
      [int]$TargetSims = 0, [double]$TargetShare = 0.25, [double]$Entropy = 0)
...
$args = @('-m', 'neural.modal_loop', $Init, "$Gen", "$K", "$Games", "$Steps", "$Batch", "$Lr", "$Window", "$MinNew", "$Sims", "$ArenaEvery", "$ArenaLag", $Shapes, "$TargetSims", "$TargetShare", "$Entropy")
